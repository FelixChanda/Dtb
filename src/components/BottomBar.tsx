...
export const BottomBar: React.FC<BottomBarProps> = ({
  ...
  return (
    <nav
      ...
      className="fixed bottom-0 inset-x-0 z-40 bg-white/65 dark:bg-slate-950/65 backdrop-blur-md border-t border-slate-200/60 dark:border-slate-800/60 shadow-2xl transition-all duration-200 pb-safe"
    >
      <div className="max-w-2xl mx-auto px-1.5 sm:px-4 py-1 flex items-center justify-between gap-0.5 sm:gap-1.5">
        ...
        >
          ...
          <span className="text-[9.5px] sm:text-[11px] font-semibold mt-0.5 tracking-tight truncate w-full text-center">
            {showBookmarksOnly ? 'Saved' : 'Saved'}
          </span>
        </button>

        {/* 3. Center Quick Action: Add Resource / Document (Thumb Reach) */}
        <button
          id="bottom-btn-add"
          onClick={onOpenAddModal}
          className="flex-1 flex flex-col items-center justify-center py-1 px-0.5 rounded-xl text-teal-700 dark:text-teal-300 hover:bg-teal-50 dark:hover:bg-teal-950/60 transition-all cursor-pointer group min-w-0"
          title="Upload or Contribute Resource"
        >
          <div className="p-1 rounded-lg bg-teal-600 dark:bg-teal-500 text-white shadow-xs group-hover:scale-105 transition-transform">
